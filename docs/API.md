...
| 场景 | 步骤 | 对外 API | 请求体要点 |
|------|------|----------|------------|
| 注册：唯一标识+密码 | 一步 | `POST /api/v1/register` | `credentialType`, `credential`, `password` |
| 注册：邮箱+验证码 | ①发码 ②提交 | `POST /api/v1/register/email_code` → `POST /api/v1/register` | `email` → `email`, `code` |
| 注册：手机+验证码 | ①发码 ②提交 | `POST /api/v1/register/phone_code` → `POST /api/v1/register` | `phone` → `phone`, `code` |
| 登录：唯一标识+密码 | 一步 | `POST /api/v1/login` | `credential`, `password` |
| 登录：邮箱+验证码 | ①发码 ②提交 | `POST /api/v1/login/email_code` → `POST /api/v1/login` | `email` → `email`, `code` |
| 登录：手机+验证码 | ①发码 ②提交 | `POST /api/v1/login/phone_code` → `POST /api/v1/login` | `phone` → `phone`, `code` |

> **注册第二步**：邮箱/手机验证码注册完成后，统一使用 `POST /api/v1/register`（非 `/login`）。  
> **登录第二步**：验证码登录统一使用 `POST /api/v1/login`（非 `/register`）。
...
|------|------|------|------|------|------------|----------------------|----------|
| 是 | POST | `/api/v1/register` | 唯一标识+密码注册 | 无 | `credentialType`, `credential`, `password` | `user-service POST /internal/user/register` → `messages/instant`；成功 → `record`（`USER_REGISTER`） | partial |
| 是 | POST | `/api/v1/register/email_code` | 注册-发送邮箱验证码 | 无 | `email` | `message-service POST /internal/message/email_code/send` | planned |
| 是 | POST | `/api/v1/register/phone_code` | 注册-发送手机验证码 | 无 | `phone` | `message-service POST /internal/message/phone_code/send` | planned |
| 是 | POST | `/api/v1/register` | 注册-邮箱/手机+验证码（第二步） | 无 | `email,code` 或 `phone,code` | `user-service POST /internal/user/register` → Message；成功 → `record`（`USER_REGISTER`） | planned |
| 是 | POST | `/api/v1/login` | 唯一标识+密码登录 | 无 | `credential`, `password` | `user-service POST /internal/user/login`；成功 → `record`（`USER_LOGIN`） | planned |
| 是 | POST | `/api/v1/login/email_code` | 登录-发送邮箱验证码 | 无 | `email` | `message-service POST /internal/message/email_code/send` | planned |
| 是 | POST | `/api/v1/login` | 登录-邮箱+验证码（第二步） | 无 | `email`, `code` | `user-service POST /internal/user/login`；成功 → `record`（`USER_LOGIN`） | planned |
| 是 | POST | `/api/v1/login/phone_code` | 登录-发送手机验证码 | 无 | `phone` | `message-service POST /internal/message/phone_code/send` | planned |
| 是 | POST | `/api/v1/login` | 登录-手机+验证码（第二步） | 无 | `phone`, `code` | `user-service POST /internal/user/login`；成功 → `record`（`USER_LOGIN`） | planned |
...
| 选修 | POST | `/api/v1/account/logout` | 登出 | Shiro | 会话 userId | `user-service POST /internal/user/{userId}/logout`；可选 `record`（`USER_LOGOUT`） | planned |
...
| 方法 | 路径 | 说明 | 鉴权 | 编排下游 | 规格状态 |
|------|------|------|------|----------|----------|
| POST | `/api/v1/admin/users` | 新增用户 | Shiro | `user-service POST /internal/user/create` → `message-service POST /internal/message/send`；成功 → `record`（`ADMIN_USER_CREATE`） | planned |
...
| 方法 | 路径 | 说明 | 鉴权 | 编排下游 | 规格状态 |
|------|------|------|------|----------|----------|
| POST | `/api/v1/templates` | 创建消息模板 | Shiro | `message-service POST /internal/message-templates`（访问日志由拦截器采集） | planned |
| GET | `/api/v1/templates` | 查询模板列表 | Shiro | `message-service GET /internal/message-templates` | planned |
| PUT | `/api/v1/templates/{id}/status` | 模板状态变更 | Shiro | `message-service PUT /internal/message-templates`（body 含 `id`、目标状态；TopBiz 做 path 映射） | planned |

### 2.9 消息 - 模板变量（选修，TopBiz 转发 → message-service）

| 方法 | 路径 | 说明 | 鉴权 | 编排下游 | 规格状态 |
|------|------|------|------|----------|----------|
| GET | `/api/v1/variables/schema` | 变量定义规则 | Shiro | TopBiz 聚合静态规则，或 `message-service GET /internal/variables/schema`（规划） | planned |
| POST | `/api/v1/variables` | 创建模板变量 | Shiro | `message-service POST /internal/variables` → `message-service POST /internal/message/send` | planned |
| GET | `/api/v1/variables/{variableId}` | 查询单个变量 | Shiro | `message-service GET /internal/variables/{variableId}` | planned |
| PUT | `/api/v1/variables/{variableId}` | 修改变量 | Shiro | `message-service GET /internal/variables/{variableId}` → `PUT /internal/variables/{variableId}` → 可选 `POST /internal/message/send` | planned |
| DELETE | `/api/v1/variables/{variableId}` | 删除变量 | Shiro | `message-service GET` → `DELETE /internal/variables/{variableId}` → 可选 `POST /internal/message/send` | planned |
...
|------|------|------|------|------|----------|----------|
| 是 | GET | `/api/v1/msg/carriers` | 载体列表；可筛 `channelType` | Shiro | `message-service GET /internal/msg/carriers?channelType=` | planned |
| 是 | GET | `/api/v1/msg/carriers/{id}` | 载体详情 | Shiro | `message-service GET /internal/msg/carriers/{id}` | planned |
| 是 | POST | `/api/v1/msg/carriers` | 新增载体 | Shiro | `message-service POST /internal/msg/carriers` | planned |
| 是 | PUT | `/api/v1/msg/carriers/{id}` | 修改载体 | Shiro | `message-service PUT /internal/msg/carriers/{id}` | planned |
| 是 | DELETE | `/api/v1/msg/carriers/{id}` | 删除载体 | Shiro | `message-service DELETE /internal/msg/carriers/{id}` | planned |
| 是 | POST | `/api/v1/msg/carriers/{id}/test` | 连通性测试（SMS 调腾讯 API） | Shiro | `message-service POST /internal/msg/carriers/{id}/test` | planned |
...
| 字段 | 说明 |
|------|------|
| `channelType` | `IN_APP` \| `TENCENT_SMS` \| `EMAIL`（必做三选一） |
| `templateId` | 消息模板 ID |
...
| `TENCENT_SMS` | `13800000000` | 经载体配置调腾讯云 SendSms |
...
| 必做 | 方法 | 路径 | 说明 | 鉴权 | 编排下游 | 规格状态 |
|------|------|------|------|------|----------|----------|
| 是 | POST | `/api/v1/send/instant` | 即时发送（三载体） | Shiro | `message-service POST /internal/messages/instant` | planned |
| 选修 | GET | `/api/v1/sending-records` | 查看发送记录 | Shiro | `message-service GET /internal/sending-records` | planned |
| 选修 | DELETE | `/api/v1/sending-records/{id}` | 删除发送记录 | Shiro | `message-service DELETE /internal/sending-records` | planned |
| 选修 | POST | `/api/v1/send/scheduled` | 定时发送 | Shiro | `message-service POST /internal/messages/scheduled` | planned |
...
|------|------|------|--------|---------------------|----------|
| POST | `/internal/message-templates` | 创建模板 | TopBiz | `/api/internal/v1/message-templates` | planned |
| GET | `/internal/message-templates` | 查询模板 | TopBiz | 同上 | planned |
| PUT | `/internal/message-templates` | 更新模板/状态 | TopBiz | 同上；对外 `PUT .../templates/{id}/status` 映射到此 | planned |
| POST | `/internal/message/send` | 通用通知（**已废弃**，请用 `messages/instant`） | TopBiz | deprecated | partial |
| POST | `/internal/message/email_code/send` | 邮箱验证码 | TopBiz | 保留 | planned |
| POST | `/internal/message/phone_code/send` | 手机验证码 | TopBiz | 保留 | planned |
| GET | `/internal/variables/schema` | 变量规则（规划） | TopBiz | 对外 `GET /api/v1/variables/schema` 转发 | planned |
| POST | `/internal/variables` | 持久化变量 | TopBiz | `/internal/variables/storage` | planned |
| GET | `/internal/variables/{variableId}` | 查询变量 | TopBiz | `/internal/storage/variables/{id}` | planned |
| PUT | `/internal/variables/{variableId}` | 修改变量 | TopBiz | 同上 | planned |
| DELETE | `/internal/variables/{variableId}` | 删除变量 | TopBiz | 同上 | planned |
| GET | `/internal/msg/carriers` | 载体列表 | TopBiz | 对外 `/api/v1/msg/carriers` | planned |
| GET | `/internal/msg/carriers/{id}` | 载体详情 | TopBiz | 同上 | planned |
| POST | `/internal/msg/carriers` | 新增载体 | TopBiz | 同上 | planned |
| PUT | `/internal/msg/carriers/{id}` | 修改载体 | TopBiz | 同上 | planned |
| DELETE | `/internal/msg/carriers/{id}` | 删除载体 | TopBiz | 同上 | planned |
| POST | `/internal/msg/carriers/{id}/test` | 连通性测试 | TopBiz | 同上 | planned |
| GET | `/internal/sending-records` | 发送记录 | TopBiz | `/api/internal/v1/sending-records` | planned |
| DELETE | `/internal/sending-records` | 删除记录 | TopBiz | 同上 | planned |
| POST | `/internal/messages/instant` | 即时发送；body 含 `channelType`（IN_APP/TENCENT_SMS/EMAIL） | TopBiz | `/api/internal/v1/messages/instant` | planned |
| POST | `/internal/messages/scheduled` | 定时发送 | TopBiz | `/api/internal/v1/messages/scheduled` | planned |
| POST | `/internal/scheduler/trigger` | 调度触发 | **自驱（message-service 调度器）** | `/api/internal/v1/scheduler/trigger` | planned |
...
| logservice §1.1–1.4 查询/指标/导出 | 飞书表为空；REST 以 §5 为准 | 运维 API **非课程用户 MVP** |
| logservice §1.3 指标配置、§2.4 WebSocket | API §5.4.1 占位 | **选修** |
| logservice §2.3 定时预聚合 | DATA_MODEL §4.4 占位 | **选修** |
| logservice §3 全量 metric | 代码仅部分聚合 | 见 [GAPS.md](./GAPS.md) §3 |
| 四服务 AccessLogInterceptor + Vector | logservice §4–§5、ADR §6 | **MVP 建议实现** |
